...
#for k in range(0, 1):
    ...
    with open(fn + '.txt', 'w') as fh, open(fn + '.reg', 'w') as regFile:
        ...
        fh.writelines('# ked exposure of Abell cluster '+clusters[k]+' mosaics in u, g, and r band taken by Dark  #\n')
        fh.writelines('# Energy Camera (DECam) mounted at the prime focus of the Blanco 4-m telescope at CTIO over #\n')
        ...
        # is_dqm_r_zero = np.ones(num_gal_r, dtype=bool)
        # pool = Pool()
        num_process = 12
        ...
        is_dqm_r_zero = np.array(is_dqm_r_zero).reshape(-1)
            ...
            #     proc = Process(target=dqm_check,
            #                    args=(ni*i, ni*(i+1), sex_cat_r[galaxy_r], hdu_r, is_dqm_r_zero, start_time))
            #     procs.append(proc)
            #     proc.start()
            #
            # for proc in procs:
            ...
            #
            # for i in range(0, num_process):
            #     args.append([i*ni, (i+1)*ni, sex_cat_r[galaxy_r], hdu_r, start_time])



        ### exclude saturated sources in stacked r band ###
        # hdu_r_stack = fits.open(work_dir + 'fits/stacked/' + stack_dqm_fn[k][2])
        num_gal_r_stack = len(sex_cat_r_stack[galaxy_r_stack])
        # is_dqm_r_zero_stack = np.ones(num_gal_r_stack, dtype=bool)
        ...
        # for i in range(0, num_process):
        #     proc = Process(target=dqm_check,
        #                    args=(ni*i, ni*(i+1), sex_cat_r_stack[galaxy_r_stack], hdu_r_stack, is_dqm_r_zero_stack,
        #                          start_time))
        #     procs.append(proc)
        #     proc.start()
        #
        ...
        coords_single = SkyCoord(sex_cat_r[galaxy_r][is_dqm_r_zero]['ALPHA_J2000'],
                                 sex_cat_r[galaxy_r][is_dqm_r_zero]['DELTA_J2000'], unit='deg')
        coords_stack = SkyCoord(sex_cat_r_stack[galaxy_r_stack][is_dqm_r_zero_stack]['ALPHA_J2000'],
                                sex_cat_r_stack[galaxy_r_stack][is_dqm_r_zero_stack]['DELTA_J2000'], unit='deg')
        idx_r_single, d2d = coords_single.match_to_catalog_sky(coords_stack)
        matched_r_single = (d2d.arcsec < max_sep)

        ref_r = vstack(sex_cat_r_stack[galaxy_r_stack][is_dqm_r_zero_stack],
                          sex_cat_r[galaxy_r][is_dqm_r_zero][~matched_r_single])
        num_stack = len(sex_cat_r_stack[galaxy_r_stack][is_dqm_r_zero_stack])
        num_all = len(ref_r)
        ...
        ### match r band again ###
        coords_r_single = SkyCoord(sex_cat_r[galaxy_r][is_dqm_r_zero]['ALPHA_J2000'],
                                   sex_cat_r[galaxy_r][is_dqm_r_zero]['DELTA_J2000'], unit='deg')
        coords_r_stack = SkyCoord(sex_cat_r_stack[galaxy_r_stack][is_dqm_r_zero_stack]['ALPHA_J2000'],
                                  sex_cat_r_stack[galaxy_r_stack][is_dqm_r_zero_stack]['DELTA_J2000'], unit='deg')
        idx_r_single, d2d_r_single = coords_r_single.match_to_catalog_sky(coords_r)
        idx_r_stack, d2d_r_stack = coords_r_stack.match_to_catalog_sky(coords_r)
        ...
        idx_u_single, d2d_u_single = coords_u_single.match_to_catalog_sky(coords_r)
        idx_u_stack, d2d_u_stack = coords_u_stack.match_to_catalog_sky(coords_r)
        ...
        idx_g_single, d2d_g_single = coords_g_single.match_to_catalog_sky(coords_r)
        idx_g_stack, d2d_g_stack = coords_g_stack.match_to_catalog_sky(coords_r)
        ...
        for i in range(0, num_all):
            if i in idx_r_single[matched_r_single]:                                      #### for r band
                ...
                r_iso_single = sex_cat_r[mag_sys][idx_match_r_single][0]
                rerr_iso_single = sex_cat_r[magerr_sys][idx_match_r_single][0]
                r_auto_single = sex_cat_r[mag_sys1][idx_match_r_single][0]
                rerr_auto_single = sex_cat_r[magerr_sys1][idx_match_r_single][0]
            else:
                r_iso_single = 999.0
            ...
            if i in idx_r_stack[matched_r_stack]:
                ...
                r_iso = sex_cat_r_stack[matched_r_stack][mag_sys][idx_match_r][0]
                rerr_iso = sex_cat_r_stack[matched_r_stack][magerr_sys][idx_match_r][0]
                r_auto = sex_cat_r_stack[matched_r_stack][mag_sys1][idx_match_r][0]
                rerr_auto = sex_cat_r_stack[matched_r_stack][magerr_sys1][idx_match_r][0]
            else:
                r_iso = 999.0
                rerr_iso = 999.0
                r_auto = 999.0
            ...
            if i in idx_u_single[matched_u_single]:                                      #### for u band
                idx_match_u_single = np.where(idx_u_single == ref_r['NUMBER'][i])
            ...
            if i in idx_u_stack[matched_u_stack]:
                idx_match_u = np.where(idx_u_stack == ref_r['NUMBER'][i])
            ...
            if i in idx_g_stack[matched_g_stack]:                                        #### for g band
                idx_match_g = np.where(idx_g_stack == ref_r['NUMBER'][i])
            ...
            if i in idx_g_single[matched_g_single]:
                idx_match_g_single = np.where(idx_g_single == ref_r['NUMBER'][i])
